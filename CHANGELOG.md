...
- added WFP ALE connect-redirect HTTPS inspection (TCP/443 + ALPN http/1.1): `SbieCapture.exe` terminates downstream TLS with a persistent per-user session CA, opens the original destination, parses HTTP/1.1, writes HAR (redacted headers, opt-in bodies), and keeps ciphertext PCAPNG; capability bits stay off until the Slice 8 live isolation suite passes
